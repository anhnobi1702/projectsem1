...
class CustomerController extends Controller
{
    ...
    function about(Request $req)
    {
        ...
        $brand = Brand::get();
        return view('customer.page.about', compact('brand'));
    }
    ...
    function productdetail(Product $pro, $id, Wishlist $wish, User $user)
    {
        ...
        if ($check_count_order > 0) {
            foreach ($check_order as $key => $value) {
                $order_detail  = OrderDetail::where('order_id', $value->id)->get();
                foreach ($order_detail as $key => $value2) {
                    $material = MaterialProduct::where('id',$value2->material_product_id)->get();
                    foreach ($material as $key => $value3) {
                        if($value3->product_id == $id){
                            $check_exist = true;
                        }
                    }
        ...
        $material = MaterialProduct::where('product_id')->get();
        $product = $pro->getbyid($id);
        $image = imageProduct::where('product_id', $id)->get();
        $size = SizeProduct::where('product_id', $id)->get();
        $material = MaterialProduct::where('product_id', $id)->get();
        $productRelated = $pro->productRelated($product->category_id);
        return view('customer.page.productdetail', compact('product', 'image', 'user','material', 'size', 'productRelated', 'value', 'reviews', 'pro', 'count_reviews','material', 'size', 'favo', 'wish', 'old_star', 'check_exist'));
    }
    ...
    function addCart(Request $req)
    {
        ...
        $materialProduct = MaterialProduct::where('product_id', $id)->get();
        foreach ($sizeProduct as $key => $value) {
            $size = $value->size_id;
        };
        foreach ($materialProduct as $key => $value) {
    ...
    function removeCart(Request $req)
    {
        ...
        $materialId = $req->input('materialId');
        if (Auth::check()) {
            Cart::where('user_id', Auth::user()->id)->where('product_id', $proId)->where('size_id', $sizeId)->where('material_id', $materialId)->delete();
        }
    }
    ...
    function post_order(OrderRequest $req, Cart $c)
    {
        ...
        foreach ($cart as $key => $value) {
            $material_product_id = MaterialProduct::where('product_id', $value->product_id)->where('material_id', $value->material_id)->first();
    ...
    function banking(Request $req)
    {
        ...
        foreach ($cart as $key => $value) {
            ...
            OrderDetail::create([
                ...
                'name' => Product::find($value->product_id)->name,
                'size_product_id' => $size_product_id->id,
                'material_product_id' => $material_product_id->id,
                'image' => Product::find($value->product_id)->image
            ]);
        }
        Cart::where('user_id', Auth::user()->id)->delete();
